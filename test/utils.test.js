...
test('retorna o "tweet pai"', () => {
  ...
  expect(getParentTweetId(tweets.orphanTweet)).toBe(null);
})
...
test('salva as informações da imagem no arquivo de setup', () => {
  ...
  const content = readFileSync(getSetupFilePath('pixelSort', infoImagem)).toString();
  expect(content).toBe(expected);
  writeFileSync(getSetupFilePath('pixelSort', infoImagem), '');
})